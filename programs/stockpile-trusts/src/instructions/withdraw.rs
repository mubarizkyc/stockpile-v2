...
use crate::{
    error::VaultError,
    util::{KLend, Stockpile},
    YieldVault,
};

pub fn withdraw(ctx: Context<Withdraw>, _project_id: u64, amount: u64) -> Result<()> {
    let project_accounts = ctx.remaining_accounts;
    // Create Kamino CPI context
    let cpi_ctx: CpiContext<'_, '_, '_, '_, klend::cpi::accounts::RedeemReserveCollateral<'_>> =
        CpiContext::new(
    ...
    // Redeem Kamino funds
    cpi::redeem_reserve_collateral(cpi_ctx, amount).map_err(|_e| {
    ...
    let contribute_ctx: CpiContext<'_, '_, '_, '_, stockpile_v2::cpi::accounts::Contribute<'_>> =
        CpiContext::new(
    ...
    contribute(contribute_ctx, _project_id, amount).map_err(|_e| {
        msg!("Kamino redemption failed.");
...
pub struct Withdraw<'info> {
    ...
    #[account(mut)]
    /// CHECK: Verified by Stockpile program
